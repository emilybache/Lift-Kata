...
public class LiftPrinter {
    public String print(LiftSystem liftSystem) {
        ...
        for (int floor : liftSystem.getFloorsInDescendingOrder()) {
            ...
            String lifts = liftSystem.getLifts()
                    .stream()
                    .map(l -> printLiftForFloor(l, floor))
                    .collect(Collectors.joining(" "));
            sb.append(lifts);
    ...
    private String printLiftForFloor(Lift lift, int floor) {
        if (lift.getFloor() == floor)
            return printLift(lift, floor);
        else {
            String padding = getWhitespace(lift.getId().length());
